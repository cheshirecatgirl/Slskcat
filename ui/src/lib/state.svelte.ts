...
export class AppState {
   ...
   * credentials were right, and the way back is one button rather than a form.
   */
  displaced = $state(false);
  /**
   * What the sign-in screen says while a sign-in nobody typed is in flight,
   * or null when the form should be showing instead.
   *
   * Kept apart from `connecting` so the two look different. An automatic
   * sign-in has nothing to fill in, so it can replace the form with a line. A
   * sign-in someone just asked for has to leave their form up: if the password
   * was wrong, taking the form away and putting it back flashes the wrong
   * thing at the moment they need to read it.
   *
   * "reconnecting" is the same wait for a session that was already working,
   * which only a proxy change causes. A route cannot be moved under an open
   * socket, so the session is rebuilt, and nothing has gone wrong.
   */
  waiting = $state<"connecting" | "reconnecting" | null>(null);
   ...
   *
   * A failed sign-in is otherwise a dead end. Switching to a name the server
   * has since deleted, or one mistyped into the switcher, ends the session
   * that was working and leaves a form with nowhere to go but forward.
   */
   ...
   *
   * The server publishes a room list but no user list, and there is no
   * message that asks for one, so "everybody" is not available and is not what
   * this is. It is the people who have actually appeared: the members of rooms
  ...
  apply(event: CoreEvent) {
    switch (event.type) {
      ...
      case "disconnected": {
        this.connected = false;
        this.connecting = false;
        // A reconnect drops the old session on the way to the new one. That
        // disconnect is a step in the sign-in rather than the end of it, so
        // the wait stays up instead of flashing the form between the two.
        if (this.waiting !== "reconnecting") this.waiting = null;
